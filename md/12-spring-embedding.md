...
In 1963, William Tutte published a paper ambitiously entitled “How to Draw a Graph”.  Let $\Sigma$ be any planar embedding of any simple planar graph $G$, and consider the following simple physical process:
...
It is not hard to see that 3-connectivity is required.  If $G$ has an articulation vertex $v$, that is, a vertex whose deletion disconnected the graph, then a Tutte drawing of $G$ can map an entire component of $G\setminus v$ to the point $p_v$.  Similarly, if $G$ has two vertices $u$ and $v$ such that $G\setminus \{u,v\}$ is disconnected, a Tutte drawing of $G$ can map an entire component of $G\setminus \{u,v\}$ to the line segment $p_up_v$.  In both cases, the Tutte drawing is not even an embedding, much less a strictly convex embedding.

## Outer Face is Outer

Whitney (1932) proved that every simple 3-connected planar graph $G$ has a _unique_ embedding on the sphere (up to homeomorphism), or equivalently, a _unique_ planar rotation system (up to a global reflection).  I will describe Whitney's proof later in this note.  Thus, in every planar embedding of $G$, the faces are bounded by the same set of cycles; we can reasonably call these cycles the _faces of $G$_.
...
Suppose each edge $uv$ is to a (first-order linear) spring with spring constant $\omega_{uv} = \lambda_{u\mathord\to v} = \lambda_{v\mathord\to u}$.  Hooke’s Law[^hooke] states that extending this spring to length $\ell$ requires force proportional to $\omega_{uv}\ell$, and therefore (by integration) the potential energy stored in the spring is $\omega_{uv}\ell^2/2$.

[^hooke]: Robert Hooke time-stamped his “true Theory of Elasticity or Springiness” in 1676 using the anagram _ceiiinosssttuv_; two years later he reveelad the unscrambled Theory _ut tensio, sic vis_ (“As the extension, so the force”).  Another anagram _abcccddeeeeefggiiiiiiiillmmmmnnnnnooprrsssttttttuuuuuuuux_ in the same 1676 publication was only revealed after Hooke’s death to encode the observation _Ut pendet continuum flexile, sic stabit contiguum rigidum inversum_ (“As hangs a flexible cable so, inverted, stand the touching pieces of an arch.”)  Anagrams and codes were a common method for 17th- and 18th-century scientists to establish priority for their results before publishing them in detail; Hooke most likely published these anagrams to avoid being scooped by Isaac Newton. 

For any vertex placement $p \in (\mathbb{R}^2)^V$, the total potential energy in the network of springs is 
$$
...
$$
for every interior vertex $u$.  In other words, all the forces on $u$ from the springs incident to $u$ cancel.  The underlying matrix of this linear system is called a weighted _Laplacian_ matrix of $G$.  This matrix is positive definite[^laplacian] and therefore non-singular, so a unique equilibrium configuration always exists.
